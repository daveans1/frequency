package com.david.frequency.vivimusic.release
...
import com.david.frequency.db.InternalDatabase
    ...
    override suspend fun doWork(): Result {
        ...
        try {
            ...
            for ((artistId, artistInfo) in artistMap) {
                ...
                // We only process carousels to avoid "Top songs" (which is a vertical list)
                // rotating old songs and triggering false new release notifications.
                for (section in artistPage.sections) {
                    if (!section.isCarousel) continue
                ...
                if (unseenReleases.isNotEmpty()) {
                    ...
                    if (trackingSince == -1L) {
                        ...
                        val validNewReleases = unseenReleases.filter { release ->
                            release.year == null || release.year >= currentYear - 1
                        }

                        Timber.tag(TAG).d(
                            "Found %d unseen releases (%d valid recent) for %s",
                            unseenReleases.size, validNewReleases.size, artistName
                        )
                        for (release in validNewReleases) {
