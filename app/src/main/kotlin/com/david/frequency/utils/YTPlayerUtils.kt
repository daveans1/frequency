...
import com.david.frequency.utils.BotDetectionMitigator
...
import com.david.frequency.utils.YTPlayerUtils.MAIN_CLIENT
import com.david.frequency.utils.YTPlayerUtils.STREAM_FALLBACK_CLIENTS
import com.david.frequency.utils.YTPlayerUtils.validateStatus
import com.david.frequency.utils.potoken.PoTokenGenerator
import com.david.frequency.utils.potoken.PoTokenResult
import com.david.frequency.utils.PlaybackLogLevel
...
object YTPlayerUtils {
     ...
     * VISIONOS doesn't require PoToken and starts instantly.
     * Note: VISIONOS has loginSupported=false, so metadata like audioConfig and
    ...
    private val STREAM_FALLBACK_CLIENTS: Array<YouTubeClient> = arrayOf(
        ...
        WEB,
        WEB_CREATOR,
        ANDROID_VR_1_61_48,  // ANDROID_VR deprecated by YouTube — kept as last resort
        ANDROID_VR_NO_AUTH,
    )
    ...
    private fun scoreCandidate(
        ...
        if (splitWantedArtists.isEmpty()) {
            ...
            for (wArtist in splitWantedArtists) {
                val matched = candAllArtists.any { cArtist ->
                    ...
                    calculateLevenshteinSimilarity(cArtist, wArtist) >= 0.82
                }
        ...
        // If artist did not match at all, hard reject
        ...
        val totalScore = (titleScore * 0.40) + (artistScore * 0.45) + (durationScore * 0.15) + explicitBonus - versionPenalty
    ...
    suspend fun playerResponseForPlayback(
        ...
        // AUTO (Streaming Only) or Low or Medium Qualities: Strictly YouTube Music (bypass external search)
        if (audioQuality == AudioQuality.AUTO || audioQuality == AudioQuality.LOW || audioQuality == AudioQuality.MEDIUM) {
            return resolvePlaybackDataWithRetry(videoId, playlistId, audioQuality, connectivityManager)
        }

        // For Max and High: Concurrently search JioSaavn while fetching YouTube Music metadata
        return coroutineScope {
            val ytDeferred = async(Dispatchers.IO) {
                resolvePlaybackDataWithRetry(videoId, playlistId, audioQuality, connectivityManager)
            }

            val saavnDeferred = async(Dispatchers.IO) {
                runCatching {
                    // Try to use known metadata right away to start concurrent search, fallback to waiting for YouTube
                    var titleToUse = knownTitle
                    var authorToUse = knownArtist
                    var durationToUse = expectedDuration

                    if (titleToUse.isNullOrBlank()) {
                        val ytResult = ytDeferred.await().getOrNull() ?: return@runCatching null
                        titleToUse = ytResult.videoDetails?.title
                        authorToUse = ytResult.videoDetails?.author
                        durationToUse = ytResult.videoDetails?.lengthSeconds?.toIntOrNull()
                    }

                    val cleanTitle = sanitizeTitle(titleToUse.orEmpty())
                    if (cleanTitle.isBlank()) return@runCatching null

                    val author = sanitizeTitle(authorToUse.orEmpty())
                    ...
                    val scoredCandidates = rawSongs.map { candidate ->
                        candidate to scoreCandidate(
                            ...
                            expectedDuration = durationToUse,
                ...
                var streamUrl = selectedStream?.first
                var streamBitrateKbps = selectedStream?.second ?: targetKbps
                if (streamUrl.isNullOrBlank()) return@runCatching null

                var contentLength = SaavnService.getContentLength(streamUrl)

                if (contentLength == null && streamBitrateKbps > 160) {
                    Timber.tag(TAG).w("Saavn: ${streamBitrateKbps}kbps URL failed validation, falling back to 160kbps")
                    val fallback160 = SaavnService.selectBestUrl(bestSong.downloadUrl, "160kbps")
                        ?: SaavnService.getBestStreamUrl(bestSong.id, "160kbps")
                    if (!fallback160.isNullOrBlank()) {
                        val fallbackLength = SaavnService.getContentLength(fallback160)
                        if (fallbackLength != null) {
                            streamUrl = fallback160
                            streamBitrateKbps = 160
                ...
                if (contentLength == null) {
                    Timber.tag(TAG).e("Saavn: Selected stream URL failed validation completely, aborting Saavn match")
            ...
            val saavnPlayback = saavnDeferred.await().getOrNull()

            if (saavnPlayback != null) {
                // If Saavn succeeds, we still need audioConfig and playbackTracking from YouTube!
                // Wait for YouTube to finish so we can attach them to saavnPlayback
                val ytResult = ytDeferred.await().getOrNull()
                Result.success(
                    saavnPlayback.copy(
                        audioConfig = ytResult?.audioConfig ?: saavnPlayback.audioConfig,
                        playbackTracking = ytResult?.playbackTracking ?: saavnPlayback.playbackTracking,
                        videoDetails = ytResult?.videoDetails ?: saavnPlayback.videoDetails
                    )
                )
            } else {
    ...
    private suspend fun resolvePlaybackData(
        ...
        // Generate PoToken ONLY if MAIN_CLIENT uses it (which it now doesn't since we use VISIONOS)
        ...
        // Try MAIN_CLIENT (VISIONOS) for fast stream resolution and METADATA_CLIENT (WEB_REMIX) for
    ...
    private fun findFormat(
        ...
        val format = when (audioQuality) {
            AudioQuality.MAX, AudioQuality.HIGH -> {
                // Tier: Highest available YouTube format
                formats.maxByOrNull {
                    it.bitrate + (if (it.mimeType.startsWith("audio/webm")) 10240 else 0)
                }
            }
            AudioQuality.MEDIUM -> {
                // Tier: Target 128 kbps (itag 140) -> fallback strictly <= 128 kbps
                val exact140 = formats.find { it.itag == 140 }
                if (exact140 != null) {
                    exact140
                } else {
                    formats.filter { it.bitrate <= 128_000 }
                        .maxByOrNull { it.bitrate + (if (it.mimeType.startsWith("audio/webm")) 10240 else 0) }
                        ?: formats.minByOrNull { it.bitrate }
                }
            }
            AudioQuality.LOW -> {
                // Tier: Target 70 kbps (itag 250) -> fallback strictly <= 70 kbps
                val exact250 = formats.find { it.itag == 250 }
                if (exact250 != null) {
                    exact250
                } else {
                    formats.filter { it.bitrate <= 70_000 }
                        .maxByOrNull { it.bitrate + (if (it.mimeType.startsWith("audio/webm")) 10240 else 0) }
                        ?: formats.minByOrNull { it.bitrate }
                }
            }
            AudioQuality.AUTO -> {
                // Tier: AUTO smartly selects best tier based on network metered status and available formats
                val highFormat = formats.maxByOrNull {
                    it.bitrate + (if (it.mimeType.startsWith("audio/webm")) 10240 else 0)
                }
                val mediumFormat = formats.find { it.itag == 140 }
                    ?: formats.filter { it.bitrate <= 128_000 }
                        .maxByOrNull { it.bitrate + (if (it.mimeType.startsWith("audio/webm")) 10240 else 0) }
                val lowFormat = formats.find { it.itag == 250 }
                    ?: formats.filter { it.bitrate <= 70_000 }
                        .maxByOrNull { it.bitrate + (if (it.mimeType.startsWith("audio/webm")) 10240 else 0) }
                    ?: formats.minByOrNull { it.bitrate }

                val targetFormat = if (connectivityManager.isActiveNetworkMetered) {
                    mediumFormat ?: lowFormat ?: highFormat
                } else {
                    highFormat ?: mediumFormat ?: lowFormat
                }
                
                // Use validateStreamSpeed to test network health if direct URL is available
                if (targetFormat != null && targetFormat.url != null) {
                    if (!validateStreamSpeed(targetFormat.url!!)) {
                        Timber.tag(logTag).d("Network too slow, downgrading quality tier from AUTO")
                        mediumFormat ?: lowFormat ?: targetFormat
                    } else {
                        targetFormat
                    }
                } else {
    ...
    @Volatile
    private var lastSpeedCheckResult = true
    private const val SPEED_CHECK_CACHE_DURATION_MS = 30_000L

    private fun validateStreamSpeed(url: String): Boolean {
        val now = System.currentTimeMillis()
        if (now - lastSpeedCheckTime < SPEED_CHECK_CACHE_DURATION_MS && lastSpeedCheckResult) {
            return true
        }
        Timber.tag(logTag).d("Validating stream speed (Micro-benchmark)")
        return try {
            val requestBuilder = okhttp3.Request.Builder()
                .get()
                .url(url)
                .header("Range", "bytes=0-10240") // Download ~10KB chunk
                .header("User-Agent", YouTubeClient.USER_AGENT_WEB)

            val start = System.currentTimeMillis()
            val response = httpClient.newCall(requestBuilder.build()).execute()
            response.body?.bytes() // consume the body
            val time = System.currentTimeMillis() - start
            val isSuccessful = response.isSuccessful
            val passed = isSuccessful && time < 800
            lastSpeedCheckTime = now
            lastSpeedCheckResult = passed
            Timber.tag(logTag).d("Stream speed validation: took ${time}ms (${if (passed) "Passed" else "Failed"})")
            passed
        } catch (e: Exception) {
            Timber.tag(logTag).e(e, "Stream speed validation failed with exception")
            lastSpeedCheckTime = now
            lastSpeedCheckResult = false
            false
        }
    }
    /**
    ...
    private suspend fun findUrlOrNull(
        ...
        if (streamUrls.isNotEmpty()) {
            ...
            // NOTE: Do NOT fall back to a different-itag audio stream here.
            // The format metadata (bitrate, mimeType, codecs) was selected by findFormat()
            // for a specific itag. Returning a URL for a different itag causes a silent
            // mismatch between what ExoPlayer is told to decode and what it actually receives.
        }
